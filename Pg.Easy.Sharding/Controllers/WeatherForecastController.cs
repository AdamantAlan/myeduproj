...
namespace Pg.Easy.Sharding.Controllers
{
    [ApiController]
    [Route("[controller]")]
    public class WeatherForecastController(ILogger<WeatherForecastController> logger, IShardedDbContextFactory dbContextFactory) : ControllerBase
    {
        [HttpGet(Name = "Order")]
        public async Task<IEnumerable<Order>> Get(long customerId)
        {
            var context = dbContextFactory.CreateForKey(customerId);
        ...
        {
            var context = dbContextFactory.CreateForKey(order.CustomerId);
